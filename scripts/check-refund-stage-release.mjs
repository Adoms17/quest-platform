...
export const manifest=JSON.parse(readFileSync(new URL('../docs/tasks/BILL-REFUND-01-migrations.json',import.meta.url),'utf8').replace(/^\uFEFF/,''))
export function validateRefundRelease(history,entries=manifest){
 ...
 if([...allowed].some(version=>!seen.has(version)))throw Error('Incomplete refund release')
 return pending.sort()
}
export function verifyRefundFiles(){
 for(const item of manifest){
  const data=readFileSync(new URL('../supabase/migrations/'+item.file,import.meta.url))
  if(createHash('sha256').update(data.toString('utf8').replace(/^\uFEFF/,'').replace(/\r\n/g,'\n')).digest('hex')!==item.sha256)throw Error('Refund migration changed: '+item.file)
 }
}
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 ...
 verifyRefundFiles()
 const pending=validateRefundRelease(parseMigrationHistory(readFileSync(process.argv[2],'utf8')))
 if(process.argv.includes('--require-applied')&&pending.length)throw Error('Refund migrations not applied')
 console.log('Refund release pending: '+(pending.join(', ')||'none'))
}
