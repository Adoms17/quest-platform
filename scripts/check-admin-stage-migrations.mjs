...
export function validateAdminMigrationHistory(data, catalog = false) {
  const signature = catalog === 'order-signature'
  ...
  const allowed = signature ? new Set(['20260924020000']) : schedule ? new Set(['20260924010000']) : details ? new Set(['20260923060000']) : processing ? new Set(['20260923050000']) : consent ? new Set(['20260923040000']) : future ? new Set(['20260923030000']) : scoped ? new Set(['20260923020000']) : notice ? new Set(['20260923010000']) : recurring ? new Set(recurringReleaseMigrations) : payments ? new Set(paymentReleaseMigrations) : release ? new Set(tariffReleaseMigrations) : catalog ? new Set(['20260919010000']) : expected
