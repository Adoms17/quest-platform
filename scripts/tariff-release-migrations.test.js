...
test.skipIf(!enabled)('чистая схема staging → 53 миграции тарифного релиза',async()=>{
...
${removal}`)).toThrow('legacy promotion access must be resolved before removal')
  ...
  expect(release).toHaveLength(53)
  ...
  const suites=readdirSync(new URL('../supabase/tests/database/',import.meta.url)).filter(f=>/^(billing_discount.*|billing_trial.*|billing_tariff.*|billing_promotions|billing_free_access_controls|platform_tariff.*|platform_fixed_tariffs|platform_discount_.*|platform_payment_catalog)\.test\.sql$/.test(f))
