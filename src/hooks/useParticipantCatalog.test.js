...
vi.mock('../services/participantCatalogApi', () => ({ searchParticipantQuests: mocks.search }))
import { useParticipantCatalog } from './useParticipantCatalog'
const page = (items, more = false) => ({ items, has_more: more, next_cursor: more ? { id: items.at(-1).id } : null })
beforeEach(() => vi.resetAllMocks())
it('не запрашивает сервер для offline/локального фильтра', () => {
  const { result } = renderHook(() => useParticipantCatalog('p1', '', 'all', false, 0))
...
it('передаёт курсор, сохраняет строки при ошибке и устраняет дубликаты после retry', async () => {
  ...
  const { result } = renderHook(() => useParticipantCatalog('p1', '', 'all', true, 0))
...
it('при смене профиля немедленно убирает строки и игнорирует поздний ответ', async () => {
  ...
  const { result, rerender } = renderHook(({ profile }) => useParticipantCatalog(profile, '', 'all', true, 0), { initialProps: { profile: 'p1' } })
  await waitFor(() => expect(result.current.items).toHaveLength(1))
