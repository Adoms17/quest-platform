...
vi.mock('../services/teamCatalogApi', () => ({ searchOrganizationTeam: mocks.search }))
...
it('не запрашивает сервер без аккаунта', () => {
  const { result } = renderHook(() => useTeamCatalog(null, 'q1', 'members', 'active', '', 0))
...
it('передаёт курсор, сохраняет строки при ошибке и устраняет дубликаты после retry', async () => {
  ...
  const { result } = renderHook(() => useTeamCatalog('a1', 'q1', 'members', 'active', '', 0))
...
it('при смене профиля немедленно убирает строки и игнорирует поздний ответ', async () => {
  ...
  const { result, rerender } = renderHook(({ profile }) => useTeamCatalog('a1', profile, 'members', 'active', '', 0), { initialProps: { profile: 'p1' } })
  await waitFor(() => expect(result.current.items).toHaveLength(1))
