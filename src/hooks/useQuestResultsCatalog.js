...
import { searchQuestResults } from '../services/questResultsApi'
import { isAbortError } from '../services/requestCancellation'

const EMPTY_ITEMS = []
export function useQuestResultsCatalog(actorId, questId, completion, search, revision, sort = 'newest') {
  const enabled = Boolean(actorId && questId)
  const pageSize = 25
  const fetchPage = useMemo(() => (options, signal) => searchQuestResults(questId, { ...options, completion, sort }, signal), [questId, completion, sort])
  const key = useMemo(() => ({ actorId, search, completion, enabled, revision, pageSize, questId, sort }), [actorId, search, completion, enabled, revision, pageSize, questId, sort])
  ...
  }, [key, actorId, search, completion, enabled, pageSize, fetchPage])
