...
vi.mock('../services/teamCatalogApi', () => ({ searchOrganizationAudit: mocks.search }))
import { useOrganizationAudit } from './useOrganizationAudit'
...
it('не запрашивает сервер без аккаунта', () => {
  const { result } = renderHook(() => useOrganizationAudit(null, 'q1', 'all', '', 0))
...
it('передаёт курсор, сохраняет строки при ошибке и устраняет дубликаты после retry', async () => {
  ...
  const { result } = renderHook(() => useOrganizationAudit('a1', 'q1', 'all', '', 0))
...
it('при смене профиля немедленно убирает строки и игнорирует поздний ответ', async () => {
  ...
  const { result, rerender } = renderHook(({ profile }) => useOrganizationAudit('a1', profile, 'all', '', 0), { initialProps: { profile: 'p1' } })
  await waitFor(() => expect(result.current.items).toHaveLength(1))
