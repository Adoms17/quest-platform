...
import { searchOrganizationTeam } from '../services/teamCatalogApi'
import { isAbortError } from '../services/requestCancellation'

const EMPTY_ITEMS = []
export function useTeamCatalog(actorId, organizationId, kind, status, search, revision) {
  const enabled = Boolean(actorId && organizationId)
  const pageSize = 25
  const fetchPage = useMemo(() => (options, signal) => searchOrganizationTeam(organizationId, kind, { ...options, status }, signal), [organizationId, kind, status])
  const key = useMemo(() => ({ actorId, search, kind, status, enabled, revision, pageSize, organizationId }), [actorId, search, kind, status, enabled, revision, pageSize, organizationId])
  ...
  }, [key, actorId, search, kind, status, enabled, pageSize, fetchPage])
