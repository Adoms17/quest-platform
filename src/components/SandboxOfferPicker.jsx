...
export default function SandboxOfferPicker({ actorId, organizationId, offers, onCreated }) {
  ...
  return <section aria-label="Предложения тестовой оплаты" className="space-y-3 rounded-xl border bg-white p-4">
    ...
    <label className="block">Тестовое предложение<select className="mt-2 w-full rounded-lg border p-3" value={selected} disabled={busy || failed} onChange={event => setSelected(event.target.value)}>
    ...
    {offer && <><p>Период: {new Date(offer.period_start).toLocaleString('ru-RU')} — {new Date(offer.period_end).toLocaleString('ru-RU')}.</p><p>Предложение действительно до {new Date(offer.valid_until).toLocaleString('ru-RU')}. Время устройства.</p></>}
    <p>Только sandbox. Реальные деньги не списываются. Подготовка заказа не начинает оплату.</p>
    <button type="button" disabled={!offer || busy} onClick={() => void prepare()} className="rounded-lg border px-4 py-3 text-blue-700 disabled:opacity-50">{busy ? 'Подготовка…' : failed ? 'Повторить подготовку заказа' : 'Подготовить тестовый заказ'}</button>
