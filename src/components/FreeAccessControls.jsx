...
import { getTrialBrowserHash, loadFreeAccessControls, previewPromotion, readFreeAccessCommand, prepareFreeAccessCommand, sendFreeAccessCommand } from '../services/freeAccessApi'
...
export default function FreeAccessControls({ actorId, organizationId, onChanged }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      let hash = null
      try { hash = await getTrialBrowserHash() } catch { /* Trial закрыт, промокод доступен без метки. */ }
  ...
  async function send() {
    const receipt = await sendFreeAccessCommand(actorId, organizationId, code)
    if (alive.current) {
      setPending(null); setPreview(null); setCode('')
  ...
  return <section className="space-y-3 rounded-xl border bg-white p-4" aria-label="Бесплатный доступ">
    <h2 className="text-lg font-semibold">Пробный доступ и промокоды</h2>
    ...
    {pending ? <div className="space-y-3">
      ...
      {pending.kind === 'promotion' && <label className="block">Прежний промокод (если потребуется повтор)<input className="mt-1 block w-full rounded border p-3" autoComplete="off" value={code} onChange={e => setCode(e.target.value)} /></label>}
      ...
      {preview.activate_before && <p>Активировать код нужно до {date(preview.activate_before)}.</p>}
      ...
      <div className="flex flex-wrap gap-2"><button className={button} disabled={busy} onClick={() => void run(async () => {
        const command = await prepareFreeAccessCommand(actorId, organizationId, preview.revision, preview.kind,
          { target: preview.id, browserHash, code })
      ...
      {data.available ? <>
        ...
        <label className="block">Промокод<input className="mt-1 block w-full rounded border p-3" autoComplete="off" maxLength={128} value={code} disabled={busy} onChange={e => setCode(e.target.value)} /></label>
        <button className={button} disabled={busy || !code.trim()} onClick={() => void run(async () => { const result = await previewPromotion(organizationId, code); if (alive.current) setPreview({ ...result, kind: 'promotion' }) })}>Проверить промокод</button>
