...
export default function RedeemQuestAccess() {
  ...
  useEffect(() => {
    ...
    loadQuestAccessPreview(initialToken)
      ...
      .catch(nextError => {
        if (active) setError(getUserErrorMessage(
          nextError,
          'Ссылка недействительна, истекла либо предназначена другому аккаунту.',
        ))
      })
    return () => { active = false }
  }, [initialToken])

  const submit = async event => { event.preventDefault(); setLoading(true); setError(''); try { const grant = await measureOperation('redeem-quest-invitation', () => redeemQuestAccessCredential(token.trim(), participantProfileId)); setQuestId(grant.quest_id) } catch (nextError) { setError(getUserErrorMessage(nextError, 'Ссылка недействительна, истекла либо выбранный профиль недоступен.')) } finally { setLoading(false) } }
  ...
  return <div className="mx-auto max-w-2xl space-y-4 p-6"><h1 className="text-2xl font-bold">Получить доступ к квесту</h1><QuestPreview preview={preview} /><form onSubmit={submit} aria-busy={loading} className="space-y-4 rounded-xl border bg-white p-5"><ParticipantProfileSelect value={participantProfileId} onChange={setParticipantProfileId} /><label className="block">Токен приглашения<input required value={token} onChange={e => setToken(e.target.value)} disabled={loading} className="mt-1 w-full rounded-lg border p-2" /></label>{error && <p className="text-red-700">{error}</p>}<button disabled={loading || !participantProfileId} className="rounded-lg bg-blue-600 px-4 py-2 text-white disabled:opacity-50">{loading ? 'Проверка...' : 'Активировать'}</button><PendingActionStatus active={loading} text="Проверяем приглашение и выдаём доступ…" /></form></div>
}
