...
export default function ParticipantGroup() {
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const [nextProfiles, nextGroups, nextInvitations, nextSupervisors, nextAuditEvents] = await Promise.all([
        listMyParticipantProfiles(),
        listMyParticipantGroups(),
        listMyParticipantProfileInvitations(),
        listManagedParticipantSupervisors(),
  ...
  const toggleSupervision = async profile => {
    ...
    try {
      ...
      toast.error(error?.message || 'Не удалось изменить контроль')
    }
  }
  ...
  const submitInvitation = async event => {
    ...
    try {
      ...
      toast.error(error?.message || 'Не удалось создать приглашение')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const updateGroupMember = async (groupIdValue, participantProfileId, memberRole, status = 'active') => {
    ...
    try {
      ...
      toast.error(error?.message || 'Не удалось изменить состав группы')
    } finally {
  ...
  const revokeSupervisor = async supervisor => {
    ...
    try {
      ...
      toast.error(error?.message || 'Не удалось отозвать контроль')
    } finally {
      setMembershipBusy(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 p-4 sm:p-6">
      <header>
        ...
        <p className="mt-2 text-gray-600">Профили участников для самостоятельного прохождения и общего устройства.</p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <form onSubmit={submitProfile} className="rounded-xl border bg-white p-5">
          ...
          <label className="mt-3 block"><span className="text-sm font-medium">Группа</span><select value={groupId} onChange={event => setGroupId(event.target.value)} className="mt-1 w-full rounded-lg border p-3"><option value="">Без группы</option>{groups.filter(group => group.can_manage).map(group => <option key={group.group_id} value={group.group_id}>{group.group_name}</option>)}</select></label>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {profiles.map(profile => (
            <article key={profile.participant_profile_id} className="rounded-xl border bg-white p-4">
              ...
              <p className="mt-1 text-sm text-gray-500">{profile.profile_kind === 'self' ? 'Мой профиль' : ageLabels[profile.age_group]}</p>
              {profile.relationship === 'supervisor' && (
                <button type="button" onClick={() => void toggleSupervision(profile)} className="mt-3 text-sm text-blue-700 hover:underline">
                  {profile.supervision_status === 'active' ? 'Приостановить контроль' : 'Возобновить контроль'}
                </button>
              )}
            </article>
          ))}
        ...
        {groups.length === 0 ? <p className="text-sm text-gray-500">Групп пока нет.</p> : groups.map(group => (
          <article key={group.group_id} className="mb-3 rounded-xl border bg-white p-4">
                ...
                {group.members.map(member => (
                  <li key={member.participant_profile_id} className="flex flex-col justify-between gap-2 rounded-lg bg-gray-50 p-3 sm:flex-row sm:items-center">
                    <div>
                      ...
                      <span className="ml-2 text-sm text-gray-500">{member.member_role === 'leader' ? 'Руководитель' : 'Участник'}</span>
                    </div>
                    {group.can_manage && (
                      <div className="flex gap-3 text-sm">
                        ...
                        >
                          {member.member_role === 'leader' ? 'Сделать участником' : 'Назначить руководителем'}
                        </button>
                        <button
                          type="button"
                          disabled={membershipBusy}
            ...
            {group.can_manage && (() => {
              ...
              const availableProfiles = profiles.filter(profile => !memberIds.has(profile.participant_profile_id))
          ...
          <div className="space-y-2">
            {supervisors.map(supervisor => (
              <article key={`${supervisor.participant_profile_id}:${supervisor.supervisor_user_id}`} className="flex flex-col justify-between gap-2 rounded-xl border bg-white p-4 sm:flex-row sm:items-center">
                ...
                {supervisor.supervision_status !== 'revoked' && (
      ...
      <section className="rounded-xl border bg-white p-5">
        ...
        <form onSubmit={submitInvitation} className="mt-4 grid gap-3 md:grid-cols-3">
          <label><span className="text-sm font-medium">Профиль</span><select required value={invitedProfileId} onChange={event => setInvitedProfileId(event.target.value)} className="mt-1 w-full rounded-lg border p-3"><option value="">Выберите профиль</option>{profiles.filter(profile => profile.profile_kind === 'dependent' && profile.supervision_status === 'active').map(profile => <option key={profile.participant_profile_id} value={profile.participant_profile_id}>{profile.display_name}</option>)}</select></label>
        ...
        {invitations.length > 0 && <div className="mt-5 space-y-2">{invitations.map(invitation => <div key={invitation.invitation_id} className="flex flex-col justify-between gap-2 rounded-lg border p-3 sm:flex-row sm:items-center"><div><strong>{invitation.participant_display_name}</strong><p className="text-sm text-gray-500">{invitation.invitation_kind === 'claim' ? 'Отдельный аккаунт' : 'Контролирующий взрослый'} · {invitation.email} · {invitation.status === 'pending' ? 'ожидает' : invitation.status}</p></div>{savedInvitationLinks[invitation.invitation_id] && invitation.status === 'pending' && <button type="button" onClick={async () => { await navigator.clipboard.writeText(savedInvitationLinks[invitation.invitation_id]); toast.success('Ссылка скопирована') }} className="text-sm text-blue-700 hover:underline">Копировать ссылку</button>}</div>)}</div>}
      </section>
    </div>
  )
}
