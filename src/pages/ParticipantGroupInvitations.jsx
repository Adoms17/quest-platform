...
export default function ParticipantGroupInvitations({ session }) {
  const { groupId } = useParams()
  const [params] = useSearchParams()
  const archive = params.get('archive') === '1'
  return <Invitations key={`${session?.user?.id}:${groupId}:${archive}`} actorId={session?.user?.id} groupId={groupId} archive={archive} />
}
function Invitations({ actorId, groupId, archive }) {
  const [search, setSearch] = useState('')
  const [revision, setRevision] = useState(0)
  const [links, setLinks] = useState({})
  const [message, setMessage] = useState('')
  const catalog = usePeopleCatalog(actorId, archive ? 'archived-invitations' : 'invitations', search, revision, groupId)
  useEffect(() => {
    let active = true
    loadLocalSecretLinks('participant-group-invitations').then(value => { if (active) setLinks(value) })
    return () => { active = false }
  }, [revision])
  return <div className="mx-auto max-w-3xl space-y-5 break-words p-4 sm:p-6">
    <Link to={`/participants/group/${encodeURIComponent(groupId)}`} className="inline-block py-2 text-blue-700">К составу группы</Link>
    ...
    <p className="text-gray-600">{catalog.group?.name ? `${catalog.group.name}. ` : ''}Приглашения, созданные вами для этой группы.</p>
    ...
    {catalog.error && <div role="alert"><p>{catalog.denied ? 'Нет доступа к приглашениям этой группы.' : 'Не удалось загрузить приглашения.'}</p><button type="button" onClick={() => catalog.hasMore ? void catalog.loadMore() : setRevision(n => n + 1)} className="py-2 text-blue-700">Повторить</button></div>}
    ...
    <div className="space-y-3">{catalog.items.map(item => <article key={item.id} className="space-y-2 rounded-xl border bg-white p-4">
      ...
      }} className="py-2 text-blue-700">Копировать ссылку</button><InvitationQrCode value={links[item.id]} label="в группу" /></div> : <p className="text-sm text-gray-500">Ссылка не сохранена на этом устройстве.</p>)}
    </article>)}</div>
