...
export default function QuestAccess() {
  ...
  useEffect(() => {
    ...
    void loadLocalSecretLinks(linkScope).then(links => {
      if (active) setCredentialLinks(links)
    })
    return () => { active = false }
  }, [linkScope])
  ...
  return <div className="mx-auto max-w-5xl space-y-8 p-4 sm:p-6">
    ...
    <section><h2 className="mb-3 text-lg font-semibold">Способы входа</h2>{loading ? <p>Загрузка...</p> : data.credentials.map(item => { const savedSecret = credentialLinks[item.id]; const cardLabel = getCredentialCardLabel(item, savedSecret); return <div key={item.id} className="mb-2 flex flex-col justify-between gap-3 rounded-lg border bg-white p-4 sm:flex-row"><div><p className="font-medium">{labels[item.kind]}{cardLabel ? ` · ${cardLabel}` : ''}</p><p className="text-sm text-gray-600">Создано {formatDate(item.created_at)} · действует до {formatDate(item.expires_at)}</p><p className="text-sm text-gray-600">Использовано {item.redemption_count} из {item.max_redemptions ?? '∞'} · {statusLabels[item.status] || item.status}</p></div><span className="flex flex-wrap gap-4">{savedSecret && <button onClick={async () => { await navigator.clipboard.writeText(savedSecret); toast.success(item.kind === 'code' ? 'Код скопирован' : 'Ссылка скопирована') }} className="text-blue-700">{item.kind === 'code' ? 'Копировать код' : 'Копировать ссылку'}</button>}{savedSecret && item.kind !== 'code' && <InvitationQrCode value={savedSecret} label="доступа к квесту" />}{item.status === 'active' && <button onClick={async () => { await revokeQuestAccessCredential(item.id); setCredentialLinks(await removeLocalSecretLink(linkScope, item.id)); await load() }} className="text-red-700">Отозвать</button>}</span></div> })}</section>
