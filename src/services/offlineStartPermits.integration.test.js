...
test.skipIf(!enabled).each(['commit', 'rollback'])('конкурентная выдача offline-права: %s', async mode => {
  const owner = randomUUID(), quest = randomUUID(), command1 = randomUUID(), command2 = randomUUID()
  const org = `(select id from public.organizations where personal_owner_id='${owner}')`
  const call = command => `set role service_role; select set_config('request.jwt.claim.sub','${owner}',false);
    select public.prepare_offline_start_permit('${quest}','${owner}','${command}');`
  ...
  try {
    ...
    first = sql(`set application_name='permit_first_${owner}'; begin; ${call(command1)} select pg_sleep(4); ${mode};`)
    let holding = false
    for (let i = 0; i < 20; i++) {
    ...
    second = sql(`set application_name='permit_second_${owner}'; ${call(command2)}`)
    let blocked = false
    for (let i = 0; i < 20; i++) {
    ...
    const retry = await sql(call(command2))
    ...
    expect(counts.output.trim().split(/\r?\n/).slice(-3)).toEqual(['1', mode === 'commit' ? '2' : '1', '0'])
  } finally {
    await Promise.allSettled([first, second].filter(Boolean))
    const cleanup = await sql(`delete from public.offline_start_permit_requests where quest_id='${quest}';
      delete from public.offline_start_permits where quest_id='${quest}'; delete from public.quests where id='${quest}';
