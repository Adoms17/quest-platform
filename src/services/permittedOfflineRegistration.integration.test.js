...
test.skipIf(!enabled).each(['commit', 'conflict', 'legacy', 'rollback'])('конкурентная привязка offline-права: %s', async mode => {
  ...
  const call = (localId = 'one') => `set role service_role; select set_config('request.jwt.claim.sub','${owner}',false);
    select public.register_permitted_offline_attempt('${quest}','${owner}','${localId}',(select id from public.offline_start_permits where quest_id='${quest}'));`
  ...
  try {
    ...
    second = sql(`set application_name='permit_second_${owner}'; ${mode === 'legacy' ? `set role service_role; select set_config('request.jwt.claim.sub','${owner}',false); select public.register_offline_quest_attempt('${quest}','${owner}','legacy-local');` : call(mode === 'conflict' ? 'two' : 'one')}`)
    let blocked = false
    for (let i = 0; i < 20; i++) {
    ...
    if (['conflict', 'legacy'].includes(mode)) {
      expect(two.code).not.toBe(0)
      expect(two.error).toContain(mode === 'legacy' ? 'offline permit registration required' : 'offline permit already bound')
    } else {
      expect(two.code, two.error).toBe(0)
      if (mode === 'commit') expect(receipt(two).id).toBe(receipt(one).id)
      else expect(receipt(two).id).not.toBe(receipt(one).id)
    }
    const retry = await sql(call())
    expect(retry.code, retry.error).toBe(0)
    expect(receipt(retry).id).toBe(receipt(['conflict', 'legacy'].includes(mode) ? one : two).id)
    ...
    expect(counts.output.trim().split(/\r?\n/).slice(-4)).toEqual(['1', '1', '1', '1'])
  } finally {
    await Promise.allSettled([first, second].filter(Boolean))
    const cleanup = await sql(`delete from public.offline_attempt_registrations where quest_id='${quest}';
