...
test.skipIf(!enabled).each(['repeat','renewal','rollback'])('конкурентная сверка review: %s',async mode=>{
  ...
  const call=`set role service_role; select public.recheck_expiration_billing_review('${confirmation}');`
  let first,second
  try {
    const setup=await sql(`insert into auth.users(id,email) values('${owner}','${owner}@example.test');
      ...
      period_start='${start}',period_end='${start}'::timestamptz+interval '1 hour' where organization_id=${org}; select public.enqueue_billing_confirmation(${org},'${confirmation}',1,(select id from public.billing_plan_versions where plan_key='pro' and version=1),'${start}','${end}'); select public.record_organization_subscription_expiration(${org},1); update public.billing_confirmation_inbox set state='review',reason='revision_conflict' where confirmation_id='${confirmation}';`)
    ...
    if(mode==='renewal') {
      ...
      expect(two.error).toContain('billing review evidence mismatch')
    } else {
    ...
    const state=await sql(`select revision||':'||status from public.organization_subscriptions where organization_id=${org};
      select count(*) from public.billing_review_resolutions where confirmation_id='${confirmation}';`)
    expect(state.code,state.error).toBe(0)
    expect(state.output.trim().split(/\r?\n/).slice(-2)).toEqual(mode==='renewal'?['3:expired','0']:['3:active','1'])
  } finally {
