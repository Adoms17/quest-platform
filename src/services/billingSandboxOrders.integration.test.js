...
test.skipIf(!enabled).each(['retry', 'another_command'])('sandbox reservation concurrency: %s', async mode => {
  ...
  const reserve = key => `select set_config('request.jwt.claim.sub','${owner}',true);
    select public.reserve_sandbox_payment_order(${org},'${key}',0,(select id from public.billing_plan_versions where plan_key='pro' and version=1),100,'123456','https://stage.qvesta.ru','2026-09-16T00:00:00Z','2026-10-16T00:00:00Z');`
  let first, second
  try {
