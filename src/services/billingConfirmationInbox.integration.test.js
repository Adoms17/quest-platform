...
test.skipIf(!enabled).each(['repeat','renewal','rollback'])('конкурентная подтверждение периода: %s',async mode=>{
  ...
  const call=`set role service_role; select public.process_billing_confirmation('${confirmation}');`
  let first,second
  try {
    const setup=await sql(`insert into auth.users(id,email) values('${owner}','${owner}@example.test');
      ...
      period_start='${start}',period_end='${end}'::timestamptz-interval '1 day' where organization_id=${org}; select public.enqueue_billing_confirmation(${org},'${confirmation}',1,(select id from public.billing_plan_versions where plan_key='pro' and version=1),'${start}','${end}');`)
    ...
    if(mode==='renewal') {
      ...
      expect(two.output).toContain('revision_conflict')
      expect(two.output).toContain('review')
    } else {
      expect(two.code,two.error).toBe(0)
    ...
    expect(state.output.trim().split(/\r?\n/).slice(-2)).toEqual(mode==='renewal'?['2:active','0']:['2:active','1'])
  } finally {
