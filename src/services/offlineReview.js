...
export async function preserveOfflineReview(questId, profileId, localId, userId, records, state = 'needs_review', permitId = null) {
  ...
  for (let offset = 0; offset < records.length; offset += 100) {
    ...
    const rpc = permitId ? 'preserve_conflicting_offline_events' : state === 'invalid_limit' ? 'preserve_limit_rejected_offline_events' : 'preserve_closed_offline_events'
    const { data, error } = await supabase.rpc(rpc, {
      p_quest_id: questId, p_participant_profile_id: profileId,
      p_local_attempt_id: localId, p_events: events,
      ...(permitId ? { p_permit_id: permitId } : {}),
    })
