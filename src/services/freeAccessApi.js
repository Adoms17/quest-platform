...
async function hashBrowserSeed(code) {
...
export function readFreeAccessCommand(actor, org) {
  ...
  if (c.org === org && c.kind === 'promotion') return { ...c, kind: 'retired_promotion' }
  if (c.org !== org || !['trial', 'reconfirm'].includes(c.kind) || !revisionValid(c.revision)
    || typeof c.id !== 'string' || typeof c.target !== 'string'
...
export async function prepareFreeAccessCommand(actor, org, revision, kind, options) {
  if (!['trial', 'reconfirm'].includes(kind)) throw new Error('Этот вид бесплатного доступа больше не поддерживается.')
  if (!actor || !org || !revisionValid(revision) || readFreeAccessCommand(actor, org)) throw new Error('Сначала проверьте предыдущий запрос.')
  const command = { org, revision, kind, id: crypto.randomUUID(), target: options.target, ...(kind === 'trial' ? { browserHash: options.browserHash } : {}) }
...
export async function sendFreeAccessCommand(actor, org) {
  const c = readFreeAccessCommand(actor, org)
  if (!c) throw new Error('Нет сохранённого запроса.')
  if (c.kind === 'retired_promotion') throw new Error('Старый запрос промодоступа сохранён, но повторная активация отключена. Обратитесь в поддержку.')
  ...
  const names = { trial: 'request_organization_trial', reconfirm: 'reconfirm_organization_trial' }
  let receipt
  try {
