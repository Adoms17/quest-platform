...
test.skipIf(!enabled).each(['boundary', 'renewal', 'rollback'])(
  'расчёт grace после ожидания блокировки: %s', async mode => {
    const owner = randomUUID(), confirmation = randomUUID(), renewal = randomUUID()
    ...
    try {
      ...
      second = sql(`set application_name='grace_second_${owner}'; ${read}`)
      let blocked = false
      for (let i = 0; i < 20; i++) {
        const state = await sql(`select count(*) from pg_stat_activity
