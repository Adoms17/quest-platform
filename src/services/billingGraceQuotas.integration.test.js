...
test.skipIf(!enabled).each(['quest', 'team'].flatMap(kind => ['boundary', 'renewal', 'rollback'].map(mode => [kind, mode])))(
  'квота %s на границе grace: %s', async (kind, mode) => {
    ...
    const action = kind === 'quest'
      ? `update public.quests set is_open=true where id='${quest}';`
      : `update public.organization_memberships set status='active' where organization_id=${org} and user_id='${member}';`
    ...
    try {
      ...
      const setup = await sql(`insert into auth.users(id,email) values('${owner}','${owner}@example.test');
        update public.organization_subscriptions set status='active',active_quest_quota_enabled=true,team_member_quota_enabled=true,
        ...
        insert into public.quests(id,creator_id,organization_id,title,is_open) values('${quest}','${owner}',${org},'Boundary test',false);
      ...
      if (mode === 'renewal') expect(two.code, two.error).toBe(0)
        ...
        expect(two.error).toContain(kind === 'quest' ? 'quest quota unavailable' : 'team quota unavailable')
      }
      ...
      const usage = await sql(kind === 'quest'
        ? `select is_open::text from public.quests where id='${quest}';`
        : `select (status='active')::text from public.organization_memberships where organization_id=${org} and user_id='${member}';`)
