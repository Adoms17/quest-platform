...
export default function Tariffs({ client, initialVersionId = null }) {
 ...
 <dt>ID версии</dt><dd>{card.id}</dd><dt>Открытые квесты</dt><dd>{card.active_quests_limit}</dd><dt>Аккаунты команды, включая владельца</dt><dd>{card.team_members_limit}</dd><dt>Пробный доступ</dt><dd>{card.plan_key === 'free' ? 'Не применяется' : card.trial_duration_days + ' суток'}</dd><dt>Создана</dt><dd>{new Date(card.created_at).toLocaleString('ru-RU')}</dd></dl><p>{card.monthly_price_minor == null ? "Цена не задана." : new Intl.NumberFormat("ru-RU", { style: "currency", currency: "RUB" }).format(card.monthly_price_minor / 100) + " / месяц"}</p></article>}
