...
export default function Organizations({ client }) {
  ...
  function closeCard() {
    setCard(null)
    requestAnimationFrame(() => { cardTrigger.current?.focus() })
  }
  ...
  return <section><div hidden={!!card}><h1>Организации</h1><p>Доступны только организации в области вашей системной роли.</p>
    ...
    </div>{card && <OrganizationCard key={card.id} organization={card} api={api} client={client} onBack={closeCard} />}
  </section>
}
