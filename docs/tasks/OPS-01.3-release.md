...
## Выпуск на stage завершён — 22.09.2026

По явному разрешению Алексея создан и объединён PR #81 https://github.com/Adoms17/quest-platform/pull/81, commit 1e486f9, staging squash d525f16bfe7d7c9c3235d7d80e8a2d7a1fff0d6c. Основной CI 35737847629 PASS (10m59s), frontend/CodeQL PASS; Auth browser сначала упал на входе, повтор 35737847729 PASS без изменения кода.

Dry-run 35737898177 подтвердил только 20260922080000. Deploy 35739318114 применил эту миграцию; 74 SQL-проверки платежей и возвратов PASS. Сборки Cloudflare для staging SHA успешны: app f9e7b34d-c8fd-49f0-9552-55eb5b6b1df7, admin 17a8b8ff-d849-424c-8af6-8819ab721aa3. Admin версия 73bf346b-2f06-460e-8761-281ef27179ba активирована на 100%; предыдущая для отката 5777bbc6-70ce-49f6-841d-217a31b90553.

HTTP-проверка app: OrganizationBilling-D0E957KK.js содержит trial_period_already_paid и оформление flex-wrap/gap. Admin: index-S2BrnW54.js содержит сообщение об устаревшем расчёте. Реальные новые платежи/возвраты не выполнялись; флаг возвратов не включался. Production не изменён. Существующий оплаченный заказ review требует отдельного завершения разбора; выпуск защит не изменяет его исторические данные.

## Диагностика и Notion — 22.09.2026

Локально batch reconciliation различает storage_failure, payment_verification_failed и provider_unavailable в безопасном журнале. Совместимый last_error использует verification_failed для БД/проверки; исходные исключения не выводятся. 12 unit-тестов PASS, lint без ошибок с прежними warnings, build PASS. Код ещё не опубликован, отдельного представления категорий в админке пока нет.

Актуальный раздел Notion OPS-01 обновлён и перечитан: https://app.notion.com/p/3df511103a9a818185fcf575781cd9bb. Отражены выпуск PR81, полный/частичный sandbox возврат, флаги, миграция, проверки и открытый разбор второго заказа. Рекомендуется вернуть оставшийся 1 ₽ и затем аудируемо закрыть дубликат без выдачи второго периода. Остаток не возвращался, заказ не закрывался; ручное переписывание статуса исключено.

## Подготовлен возврат остатка тестового дубликата

Остаток проверен: платёж 200 коп., возвращено 100 коп., заказ review. Для ручного подтверждения владельцем включён stage ADMIN_SANDBOX_REFUNDS_ENABLED и подготовлена frontend-версия 8684cf92-c1b2-4c47-b6bf-5f18c4d842f6 (asset index-CJvjA-51.js) с исправлением устаревшего preview. Возврат остатка агентом не выполнялся. После подтверждения вернуть флаг false и обычную версию 73bf346b-2f06-460e-8761-281ef27179ba. Полный возврат сам по себе не означает исполнение подписки; аудируемое закрытие оплаченного дубликата ещё предстоит реализовать.

## Возврат остатка подтверждён

После повторного подтверждения Алексея сервер проверен: заказ e0f6defe-b7ab-4c2b-801f-e151e8cb91da, amount_minor=200, refunded_minor=200, pending_minor=0. Денежный остаток полностью возвращён. Сам заказ остаётся review: аудируемое завершение разбора ещё не реализовано, исполнение подписки не подменялось. ADMIN_SANDBOX_REFUNDS_ENABLED выключен; обычная версия админки 73bf346b-2f06-460e-8761-281ef27179ba восстановлена. Production не изменён.

## Локально: аудируемое завершение возвращённого дубликата

Добавлена миграция 20260922090000_resolve_refunded_trial_duplicate.sql: закрытый immutable/RLS журнал и private resolver перед исполнением discount payment. Только sandbox, проверенный succeeded-платёж, отсутствие исполнения доступа, существующий другой оплаченный период того же trial и тех же дат, полный подтверждённый возврат с provider_refund_id, отсутствие незавершённых возвратов. Освобождает резерв скидки, закрывает checkout как cancelled и заказ как finished; исходная подписка и платный период не изменяются. Повтор идемпотентен. Клиентского RPC нет; вызов только через существующую серверную сверку.

14 pgTAP-проверок на синтетических данных внутри откатанной локальной транзакции PASS: без возврата/частичный возврат, полный возврат, повтор, аудит, состояние заказа, сохранение подписки/первого периода, RLS/права. На stage миграция не применена и реальный review-заказ не закрыт. Перед выпуском: включить миграцию/новый тест в release-проверки, пройти полный replay и проверить отображение терминального отказа в UI.

## Подготовка выпуска закрытия дубликата
