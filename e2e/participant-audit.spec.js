...
test('UX05: журнал профиля', async ({ page }, testInfo) => {
  test.setTimeout(90000)
  const user = { id: '00000000-0000-4000-8000-000000000010', email: 'demo@example.test', aud: 'authenticated', role: 'authenticated' }
  const requests = []
  let denied = false
  const items = Array.from({ length: 40 }, (_, n) => ({ id: `i${n}`, actor_username:`Автор ${n}`,action:'invitation.created',created_at:'2026-09-14T10:00:00Z' }))
  ...
  await page.route('http://127.0.0.1:54321/**', async route => {
    ...
    if (path.endsWith('/search_participant_audit')) {
      if (denied) { await route.fulfill({ status: 403, json: { code: '42501', message: 'denied' } }); return }
      const args = route.request().postDataJSON()
      const found = items.filter(i => i.actor_username.includes(args.p_search || ''))
  ...
  await expect(page.locator('article').first()).toContainText('Создано приглашение')
  await expect(page.locator('article').nth(1)).toContainText('Автор 1')
  await page.getByRole('button', { name: 'Показать ещё', exact: true }).click()
  await expect(page.locator('article')).toHaveCount(40)
  await page.getByLabel('Найти по имени автора или участника действия').fill('Автор 39')
  ...
  await page.getByRole('heading', { name: 'Журнал управления профилем', exact: true }).scrollIntoViewIfNeeded()
  await page.screenshot({ path: testInfo.outputPath('audit.png'), scale: 'css' })
  denied = true
  await page.getByRole('button', { name: 'Обновить журнал' }).click()
