...
test('UX05: привязка аккаунта из карточки без общей загрузки', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', { name: 'Связать с аккаунтом', exact: true }).click()
  const form = page.getByRole('region', { name: 'Привязка аккаунта' })
  await form.getByRole('button', { name: 'Отмена', exact: true }).click()
  expect(creates).toHaveLength(0)
  await page.getByRole('button', { name: 'Связать с аккаунтом', exact: true }).click()
  await expect(form.getByText(/Профиль и его история сохранятся/)).toBeVisible()
  ...
  await page.screenshot({ path: testInfo.outputPath('profile-claim.png'), scale: 'css' })
  await form.getByRole('button', { name: 'Готово', exact: true }).click()
  expect(creates).toEqual([{ p_participant_profile_id: 'p1', p_invitation_kind: 'claim', p_email: 'recipient@example.test' }])
