...
test('UX05: каталог приглашений профиля', async ({ page }, testInfo) => {
  ...
  await page.route('http://127.0.0.1:54321/**', async route => {
    ...
    if (path.endsWith('/search_my_profile_invitations')) {
      ...
      data = { profile_id:'p1', items: found.slice(start,end), has_more: end < found.length, next_cursor: end < found.length ? { offset: end } : null }
    }
  ...
  }, 'participant-profile-invitations')
  ...
  await page.screenshot({ path: testInfo.outputPath('profile-invitations.png'), scale: 'css' })
  denied = true
  await page.getByRole('button', { name: 'Обновить приглашения' }).click()
  await expect(page.getByRole('alert')).toBeVisible()
  await expect(page.locator('article')).toHaveCount(0)
  expect(requests.some(path => /get_my_participant_(profiles|groups|profile_invitations)/.test(path))).toBe(false)
})
