...
it('recurring POST uses saved method and requires a server guard after shop verification', async () => {
 const events=[]
 const network=vi.fn(async (url)=>{events.push(url.endsWith('/me')?'shop':'post');return response(url.endsWith('/me')?info:{...payment,status:'succeeded',paid:true,payment_method:{id:'test-method',saved:true}})})
 const guard=vi.fn(async()=>{events.push('guard');return true})
 const result=await client(network,{beforeRecurringSend:guard}).createRecurringPayment({...order,providerMethodId:'test-method'})
 expect(result.status).toBe('succeeded')
 expect(events).toEqual(['shop','guard','post'])
 const body=JSON.parse(network.mock.calls[1][1].body)
 expect(body.payment_method_id).toBe('test-method');expect(body.amount.value).toBe('1.00')
 expect(body).not.toHaveProperty('confirmation');expect(body).not.toHaveProperty('save_payment_method')
 expect(network.mock.calls[1][1].headers['Idempotence-Key']).toBe(order.idempotencyKey)
})
it.each([false, undefined])('recurring denied guard %s cannot POST',async allowed=>{
 const network=vi.fn().mockResolvedValue(response(info))
 await expect(client(network,{beforeRecurringSend:async()=>allowed}).createRecurringPayment({...order,providerMethodId:'test-method'})).rejects.toThrow('recurring_send_not_authorized')
 expect(network.mock.calls.every(([,o])=>o.method==='GET')).toBe(true)
})
it('recurring missing guard or expired window fails closed',async()=>{
 const network=vi.fn()
 await expect(client(network).createRecurringPayment({...order,providerMethodId:'test-method'})).rejects.toThrow('recurring_send_not_authorized')
 await expect(client(network,{beforeRecurringSend:async()=>true,now:()=>Date.parse(order.firstSentAt)+23*3600000}).createRecurringPayment({...order,providerMethodId:'test-method'})).rejects.toThrow('payment_reconciliation_required')
 expect(network).not.toHaveBeenCalled()
})
it('recurring unknown POST outcome is not retried; recovery uses GET',async()=>{
 const network=vi.fn().mockResolvedValueOnce(response(info)).mockRejectedValueOnce(new Error('connection lost'))
 await expect(client(network,{beforeRecurringSend:async()=>true}).createRecurringPayment({...order,providerMethodId:'test-method'})).rejects.toThrow('payment_outcome_unknown')
 expect(network.mock.calls.filter(([,o])=>o.method==='POST')).toHaveLength(1)
 const recovery=vi.fn().mockResolvedValueOnce(response(info)).mockResolvedValueOnce(response({items:[{...payment,status:'succeeded',paid:true,payment_method:{id:'test-method',saved:true}}]}))
 expect((await client(recovery).findPayment({...order,providerMethodId:'test-method'})).status).toBe('succeeded')
 expect(recovery.mock.calls.every(([,o])=>o.method==='GET')).toBe(true)
})
it('recurring mismatched method cannot be accepted in reconciliation',async()=>{
 const network=vi.fn().mockResolvedValueOnce(response(info)).mockResolvedValueOnce(response({...payment,status:'succeeded',paid:true,payment_method:{id:'wrong-method'}}))
 await expect(client(network).readPayment({...order,providerPaymentId:id,providerMethodId:'test-method'})).rejects.toThrow('payment_order_mismatch')
})
it('recurring checks retry window again after guard latency',async()=>{
 let time=Date.parse(order.firstSentAt)
 const network=vi.fn().mockResolvedValue(response(info))
 await expect(client(network,{now:()=>time,beforeRecurringSend:async()=>{time+=23*3600000;return true}}).createRecurringPayment({...order,providerMethodId:'test-method'})).rejects.toThrow('payment_reconciliation_required')
 expect(network).toHaveBeenCalledTimes(1)
})
