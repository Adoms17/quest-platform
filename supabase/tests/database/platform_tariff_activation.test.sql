...
select ok(not has_function_privilege('authenticated','public.enable_fixed_tariff_version(uuid,uuid,uuid)','execute'),'старый этап закрыт для authenticated');
select ok(not has_function_privilege('anon','public.enable_fixed_tariff_version(uuid,uuid,uuid)','execute'),'старый этап закрыт для anon');
