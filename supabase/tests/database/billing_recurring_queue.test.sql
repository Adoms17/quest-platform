...
select is(public.list_sandbox_recurring_work('123'),'[]'::jsonb,'будущий период без попытки не отправляется заранее');
select platform_private.begin_recurring_attempt(current_setting('test.prepared')::uuid);
select is(public.list_sandbox_recurring_work('123'),jsonb_build_array(current_setting('test.prepared')),'начатая попытка доступна сверке');
select is(public.list_sandbox_recurring_work('456'),'[]'::jsonb,'другой магазин изолирован');
...
select is(public.list_sandbox_recurring_work('123'),'[]'::jsonb,'повтор сверки ждёт окончания паузы');
set local role authenticated;
select throws_ok($$select public.list_sandbox_recurring_work('123')$$,'42501',null,'клиент не читает очередь');
reset role;
select ok(has_function_privilege('service_role','public.list_sandbox_recurring_work(text)','execute'),'сервер читает очередь');
