begin;
select no_plan();
insert into auth.users(id,email) values(md5('cancel-evaluation-owner')::uuid,'cancel-evaluation-owner@example.test');
select set_config('test.eval_org',(select id::text from public.organizations where personal_owner_id=md5('cancel-evaluation-owner')::uuid),true);
...
update public.organization_subscriptions set cancel_at_period_end=false where organization_id=current_setting('test.eval_org')::uuid;
...
select set_config('request.jwt.claim.sub',md5('cancel-evaluation-owner')::uuid::text,true);
...
select is((select string_agg(distinct intent_kind,',') from public.billing_intent_evaluations where organization_id=current_setting('test.eval_org')::uuid),'cancel_renewal','audit identifies cancellation');
select * from finish();
rollback;
