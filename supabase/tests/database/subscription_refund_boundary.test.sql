...
select throws_ok($$select platform_private.apply_subscription_refund(current_setting('test.request')::uuid)$$,'55000','subscription refund not confirmed','unconfirmed refund rejected across boundary');
select is((select status from public.organization_subscriptions where organization_id=current_setting('test.org')::uuid),'trial','failed application rolls back trial advancement');
select is((select count(*)::integer from public.billing_trial_transitions where access_id=(current_setting('test.trial')::jsonb->>'trial_access_id')::uuid and kind='finished'),0,'failed application leaves no finished transition');
...
select set_config('test.applied',platform_private.apply_subscription_refund(current_setting('test.request')::uuid)::text,true);
...
select is(platform_private.apply_subscription_refund(current_setting('test.request')::uuid)::text,current_setting('test.applied'),'retry is stable');
