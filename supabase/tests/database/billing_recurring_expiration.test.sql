...
select is(platform_private.apply_recurring_period(current_setting('test.prepared')::uuid)->>'state','applied','оплата применяется после технического истечения');
select is((select status from public.organization_subscriptions where organization_id=current_setting('test.org')::uuid),'active','подписка активна');
