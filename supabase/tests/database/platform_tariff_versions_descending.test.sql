...
select set_config('test.page',public.read_platform_tariff_versions('zz_group')::text,true);
select is(jsonb_array_length(current_setting('test.page')::jsonb->'items'),25,'страница ограничена 25 версиями');
select is(current_setting('test.page')::jsonb#>>'{items,0,version}','30','первой идёт самая поздняя версия');
select is(current_setting('test.page')::jsonb#>>'{items,24,version}','6','сортировка убывающая, не по UUID');
select set_config('test.next',public.read_platform_tariff_versions('zz_group',current_setting('test.page')::jsonb->'next_cursor')::text,true);
select is(jsonb_array_length(current_setting('test.next')::jsonb->'items'),5,'оставшиеся версии на следующей странице');
select is(current_setting('test.next')::jsonb#>>'{items,0,version}','5','граница без дубликатов');
select set_config('request.jwt.claims','{"aal":"aal1"}',true);
select throws_ok($t$select public.read_platform_tariff_versions('zz_group')$t$,'42501','platform access denied','MFA обязательна');
reset role;
