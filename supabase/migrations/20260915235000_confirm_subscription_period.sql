create table public.billing_period_confirmations (
...
alter table public.billing_period_confirmations enable row level security;
revoke all on public.billing_period_confirmations from public,anon,authenticated;

create function public.confirm_organization_subscription_period(
...
declare s public.organization_subscriptions%rowtype; target public.billing_plan_versions%rowtype;
  ...
  request jsonb; previous jsonb; result jsonb;
begin
  ...
  insert into public.billing_period_confirmations(confirmation_id,organization_id,request,before_state,result)
    values(p_confirmation_id,p_organization_id,request,previous,result);
  return result;
end;
$$;
