-- Доступность действий — подсказка UI; существующие RPC повторно проверяют условия.
create or replace function public.search_participant_supervisors(p_participant_profile_id uuid,p_search text default '',p_after jsonb default null,p_limit integer default 25)
...
begin
  ...
  v_restore := exists(select 1 from public.participant_profiles p where p.id=p_participant_profile_id and p.profile_kind='dependent' and p.status='active' and p.created_by_user_id=auth.uid())
    and not exists(select 1 from public.participant_profile_accounts a where a.participant_profile_id=p_participant_profile_id and a.relationship='self' and a.status='active')
    and not exists(select 1 from public.participant_supervisions s where s.participant_profile_id=p_participant_profile_id and s.status in ('active','suspended'));
  with page as (
    select s.supervisor_user_id id,coalesce(u.username,'') username,a.email::text email,s.status,
      ...
      s.status in ('active','suspended') and (v_manage or s.supervisor_user_id=auth.uid()) can_revoke,
      v_restore and s.supervisor_user_id=auth.uid() and s.status='revoked' can_restore
